...
public class Animal : LivingEntity
{
    ...
    protected virtual void Update()
    {
        if (dead)
        {
            this.Consume(decay * Time.deltaTime); // decay over time
        }
        ...
        else
        {
            ...
            if (hunger >= 1)
            {
                Kill(CauseOfDeath.Hunger);
            }
            else if (thirst >= 1)
            {
                Kill(CauseOfDeath.Thirst);
            }
        }

    }

    protected override void Kill(CauseOfDeath cause)
    {
        if (!dead)
        {
            dead = true;
    ...
    protected virtual void ChooseNextAction()
    {
        ...
        if (hunger < 0.1 && thirst < 0.1)
        {
            // we are happy; no need to search for food or water 
            FindMate();
    ...
    protected void Act()
    {
        switch (currentAction)
        {
            ...
            case CreatureAction.GoingToFood:
                ...
                {
                    currentAction = CreatureAction.Eating;  // Food disapeard before reaching it 
                }
                break;
    ...
    {
        if (target != coord && !dead)
        {
            transform.rotation = Quaternion.LookRotation(target - coord);
        }
    }
